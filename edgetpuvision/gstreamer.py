...
def run_gen(render_overlay_gen, *, source, loop, display):
    inference_size = render_overlay_gen.send(None)  # Initialize. set yield utils.input_image_size(engine) none
    next(render_overlay_gen)   #excute to detect.py  yield utils.input_image_size(engine)
